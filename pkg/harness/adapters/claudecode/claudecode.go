...
// ParseOutcome extracts usage/session from the envelope but deliberately
// reports no outcome: the forge poll (PR found → pr_opened) and exit-code
// heuristics stay authoritative. The orchestrator merges Usage into
...
func (a *Adapter) ParseOutcome(_ harness.TaskSpec, res harness.ExecResult) (harness.OutcomeReport, error) {
	...
	if err := json.Unmarshal(res.Stdout, &env); err != nil {
		return harness.OutcomeReport{}, fmt.Errorf("decode claude result envelope: %w", err)
	}
	if env.Type != "result" {
		return harness.OutcomeReport{}, nil
	}
