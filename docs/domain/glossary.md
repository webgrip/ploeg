...
The normalized tracker event that names a Team for an ingested Work Item, transitioning it to queued. Assigning in the tracker is the sole human gesture that puts work in front of agents.

**See also:** [Tracker Event](#tracker-event), [Team Queue](#team-queue)  
...
A Work Item created from a Forge Event (review submitted, check failed, merge-state dirty) rather than from an Assignment. It references its source PR and Work Item, is routed to the Team owning the source branch, and enters the lifecycle directly at queued — there is no Tracker Item to mirror at creation time.

**See also:** [Work Item](#work-item), [Forge Event](#forge-event), [Team](#team)  

## Forge Event
*Context: Integration*

The normalized result of parsing a forge webhook — review_submitted, check_failed, or merge_state_dirty — with repo, PR, branch, and the feedback body for classification. Source of every Follow-Up.

**See also:** [Follow-Up](#follow-up), [Forge Provider](#forge-provider)  

## Forge Provider
*Context: Integration*

The SPI adapter for one git forge: verify and parse webhooks into normalized Forge Events and write back PR comments. Reference implementation: Forgejo.

**See also:** [Tracker Provider](#tracker-provider), [Forge Event](#forge-event)  
...
The input contract of an Agent Container: Work Item snapshot, Role, optional Checkpoint, repo/forge endpoints. Injected as a file mount or environment; deliberately credential-free.

**See also:** [Outcome Report](#outcome-report), [Agent Container](#agent-container)  
...
The normalized result of parsing a tracker webhook — assigned, updated, or unassigned — with the external id and resolved Team. The core only ever sees normalized events, never raw vendor payloads.

**See also:** [Assignment](#assignment), [Tracker Provider](#tracker-provider)  
...
**See also:** [Tracker Item](#tracker-item), [Lease](#lease), [Follow-Up](#follow-up)  
