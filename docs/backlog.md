...
97. **ADRs** — record the already-made decisions (Go, Apache-2.0, KEDA-as-default, lease model, Postgres-only state, provider-neutrality rule) in MADR format so future contributors inherit the *why*. **Bootstrapped 2026-07-29**: [adr/](adr/index.md) (MADR 4.0.0, template + registry index) with ADR-0001–0003 covering the Work Target decoupling. Still open: the retro-ADRs listed above, and wiring `validate_adr_consistency.py` into `.forgejo/workflows/on_pull_request.yml` so file↔registry drift cannot land.
